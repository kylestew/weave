...
A generative application that simulates an endlessly weaving loom. The three components of a weaving draft — threading, tie-up, and treadling — independently evolve over time, producing a continuously scrolling fabric that never repeats.
...
The program simulates a 4-shaft floor loom with 64 warp ends. At each tick, it produces one new row of fabric by evaluating the standard drawdown formula:
...
Where:
- `x` is the column (0–63), representing a warp end
- `y` is the current pick number (monotonically increasing, never resets)
- `Threading[x]` returns a shaft number (0–3) for warp end x
- `Treadling[y]` returns a treadle number (0–3) for pick y
- `Tieup[treadle][shaft]` returns 1 (warp on top) or 0 (weft on top)

The result is a 64-wide binary row: each cell is either warp-visible or weft-visible. With a color sett applied, this becomes the fabric's visual output.

### Constraints

The simulation respects real loom constraints to keep output looking like plausible fabric:

- **4 shafts, 4 treadles** — the sweet spot for handweaving. Enough complexity for twills, overshot, and satin-like structures, but constrained enough to feel woven rather than arbitrary.
- **No impossible sheds** — all-up and all-down are excluded from the tie-up (these produce no interlacement and aren't physically weave-able).
...
**What it is:** An array of 64 shaft assignments (values 0–3), one per warp end.

**How it evolves:** Threading changes are the most dramatic — on a real loom, re-threading is a major operation. The program reflects this by changing threading infrequently and abruptly.
...
A curated list of classic threadings, each defined as a repeating unit that tiles across 64 ends:
...
**What it is:** A 4x4 boolean matrix. `Tieup[treadle][shaft]` = 1 means pressing that treadle raises that shaft.
...
**What it is:** A repeating sequence that determines which treadle is pressed for each pick. The sequence has a period (length) and cycles.
...
- **Tromp-as-writ:** Periodically, the treadling switches to **tromp-as-writ** mode — the treadling sequence mirrors the threading pattern. This is a traditional technique that produces symmetrical, often surprising results.
...
- **Warp color:** determined by the column position `x` and the current warp color sett
- **Weft color:** determined by the current pick's weft color
...
A color sett is a repeating sequence of color indices applied across the warp. Classic setts:
...
- **Warp sett** changes infrequently (aligned with threading changes) since on a real loom, the warp colors are fixed at setup time.
...
- **Top:** Threading grid (64 columns x 4 rows, showing shaft assignments)
- **Top-right corner:** Tie-up matrix (4x4)
- **Right side:** Treadling sequence (the last 64 picks, 4 columns x 64 rows)
...
- **Overshot simulation** — alternating tabby and pattern picks with two weft colors
- **Sound** — map treadling rhythm to audio pulses; each shaft could have a pitch
- **WIF export** — save the generated draft as a standard WIF file that could be woven on a real loom
...
Weave is a meditation on pattern, constraint, and emergence. By faithfully simulating the mechanics of a 4-shaft loom — and then letting its three independent components drift — it produces an endlessly unfolding textile that is always structured, always surprising, and always recognizably *woven*.
